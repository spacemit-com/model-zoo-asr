...
std::vector<float> convertToMonoFloat(const AudioChunk& audio) {
    ...
    } else if (audio.format == AudioFormat::PCM_F32LE) {
...
std::string languageCode(Language language) {
    return language == Language::AUTO ? std::string() : languageToString(language);
}
...
bool addMimeField(curl_mime* mime, const char* name, const std::string& value) {
    ...
    return curl_mime_name(part, name) == CURLE_OK &&
...
ErrorInfo FunASRBackend::transcribe(const std::vector<float>& samples, std::string& out_text) {
    const std::string wav = wavEncode(samples, config_.sample_rate);
    if (wav.empty()) {
    ...
    if (!file_part) {
        curl_mime_free(mime);
        curl_easy_cleanup(curl);
        return ErrorInfo::error(ErrorCode::INTERNAL_ERROR, "curl_mime_addpart failed");
    }
    const bool file_ok =
    ...
    bool mime_ok = file_ok &&
        addMimeField(mime, "model", model_) &&
        addMimeField(mime, "response_format", "json") &&
        addMimeField(mime, "temperature", "0") &&
        addMimeField(mime, "max_tokens", "512");
    const std::string language = languageCode(config_.language);
    if (!language.empty()) {
        mime_ok = mime_ok && addMimeField(mime, "language", language);
    }
    if (!mime_ok) {
        curl_mime_free(mime);
        curl_easy_cleanup(curl);
...
ErrorInfo FunASRBackend::recognize(const AudioChunk& audio, RecognitionResult& result) {
    ...
    auto mono = convertToMonoFloat(audio);
    if (mono.empty()) {
...
ErrorInfo FunASRBackend::recognizeFile(
    ...
    const int64_t audio_ms = frames_read * 1000 / info.samplerate;
    auto model_audio = audio_utils::normalizeSampleRate(
        std::move(mono), info.samplerate, config_.sample_rate);
    if (model_audio.empty()) {
        return ErrorInfo::error(ErrorCode::INVALID_CONFIG, "Empty audio after resampling");
    }
